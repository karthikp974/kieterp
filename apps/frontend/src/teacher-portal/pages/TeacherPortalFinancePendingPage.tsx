...
import type { HtpoFinanceStudentRow } from "../htpo-finance-types";
...
function PendingStudentsContent() {
  ...
  const [items, setItems] = useState<HtpoFinanceStudentRow[]>([]);
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await authFetch(`/api/portals/teacher/finance/pending-students?${query}`);
      if (!res.ok) throw new Error(await readApiError(res, "Could not load pending students."));
      const data = (await res.json()) as { items: HtpoFinanceStudentRow[]; total: number };
      setItems(data.items);
      setTotal(data.total);
    } catch (error) {
    ...
    <div className="htpo-finance-subpage">
      <header className="htpo-finance-subhead">
        <div>
          ...
          <p>Students with outstanding fee balance in your scope.</p>
        </div>
      ...
      <div className="htpo-finance-table-wrap">
        ...
        {!loading && !items.length ? <p className="htpo-finance-empty">No pending students.</p> : null}
      </div>
